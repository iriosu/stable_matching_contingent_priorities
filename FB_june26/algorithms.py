...
def _rebuild_school_priorities(pref_original, z, colleges):
    """Rebuild ≻_c^t so that students with z[s][c]=1 precede those with z[s][c]=0;
    ties within each tier are broken by the original ≻_c."""
    out = copy.deepcopy(pref_original)
    for c in colleges:
        if c not in pref_original:
            continue
        ranked = sorted(
            pref_original[c].items(),
            # primary key: -z (1 → -1 sorts before 0 → 0)
            # secondary key: original rank in pref[c]
            key=lambda item: (-z.get(item[1], {}).get(c, 0), item[0]),
        )
        out[c] = {p + 1: stud for p, (_, stud) in enumerate(ranked)}
    return out


def Simultaneous(inputs, max_iter=200):
    """Simultaneous (monotone) heuristic.

    Runs student-proposing DA over ALL students at once. After each DA run,
    every sibling of a matched student gets z=1 at that school. Priority
    elevations are CUMULATIVE: once z[s,c]=1, it is never revoked.

    Implements absolute contingent priority via per-iteration ≻_c^t updates.
    Convergence is guaranteed in finite time because z is monotone
    non-decreasing and the set {(s,c) : z[s,c]=1} is bounded by S × C.
    """
    students, colleges, pref, cap, siblings, levels, students_per_level = inputs

    stime = time.time()
    z = {s: {pref[s][k]: 0 for k in pref[s]} for s in students}
    ...
    while t < max_iter:
        match = DA(students, pref_updated, cap)

        if prev_match is not None and all(match[s] == prev_match[s] for s in students):
            break  # converged

        # Monotone update: only set z=1, never reset
        for s, c_match in match.items():
            if c_match is None:
                continue
            rbd = c_match.split("_")[0]
            for sib in siblings.get(s, []):
                if sib not in z:
                    continue
                # Boost sib at every school in their pref list with the same RBD
                for p in pref.get(sib, {}):
                    sib_school = pref[sib][p]
                    if sib_school.split("_")[0] == rbd:
                        z[sib][sib_school] = 1

        pref_updated = _rebuild_school_priorities(pref, z, colleges)
        prev_match = match
        t += 1

    x_opt = {s: {match[s]: 1} for s in students if match[s] is not None}
    status = "completed" if t < max_iter else "max_iter_reached"
    return {
        "status": status, "x_opt": x_opt,
...
def SimultaneousNonMono(inputs, max_iter=200):
    """Simultaneous non-monotone variant.

    Same outer loop as Simultaneous, but z is RECOMPUTED from scratch at each
    iteration based on the current matching μ^t. If a student s was matched to
    school c in iteration t but no longer in iteration t+1, the priority that
    s's siblings had at c is REVOKED.

    Because z is no longer monotone, convergence is not guaranteed. The
    algorithm halts in one of three ways:
      - 'completed':       two consecutive matchings coincide;
      - 'cycle_detected':  current match equals a strictly earlier match;
      - 'max_iter_reached': iteration cap hit.
    """
    students, colleges, pref, cap, siblings, levels, students_per_level = inputs
    ...
    while t < max_iter:
        ...
        signature = frozenset((s, match[s]) for s in students)
        if signature in seen_signatures:
            status = "cycle_detected"
            break
        seen_signatures.add(signature)

        # NON-MONOTONE: rebuild z from current match only
        z = {s: {pref[s][k]: 0 for k in pref[s]} for s in students}
        # Pass 1: receives — sibling of s is at RBD(c)
        for s, c_match in match.items():
            if c_match is None:
                continue
            rbd = c_match.split("_")[0]
        ...
        # Pass 2: provides — s is at RBD(c) and a sibling weakly prefers RBD(c)
        for s, c_match in match.items():
            if c_match is None or s not in z:
                continue
            rbd = c_match.split("_")[0]
            # Does some sibling weakly prefer this RBD to their current match?
            for sib in siblings.get(s, []):
                sib_match = match.get(sib)
                # Find best rank of any RBD-c school in sib's pref
                best_rank_in_rbd = None
                sib_prefs = pref.get(sib, {})
                for k in sorted(sib_prefs):
                    if sib_prefs[k].split("_")[0] == rbd:
                        best_rank_in_rbd = k
                        break
                if best_rank_in_rbd is None:
                    continue  # sib doesn't list this RBD
                sib_match_rank = None
                if sib_match is not None:
                    for k in sorted(sib_prefs):
                        if sib_prefs[k] == sib_match:
                            sib_match_rank = k
                            break
                # Sibling weakly prefers RBD(c) iff their best RBD-c rank is ≤ their current rank,
                # or they're unassigned.
                if sib_match is None or (sib_match_rank is not None
                                        and best_rank_in_rbd <= sib_match_rank):
                    # s provides priority at RBD(c) → boost s in their own pref list at this RBD
                    for p in pref.get(s, {}):
                        s_school = pref[s][p]
                        if s_school.split("_")[0] == rbd:
                            z[s][s_school] = 1
                    break  # one provider witness is enough
