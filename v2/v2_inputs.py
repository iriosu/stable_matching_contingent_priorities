...
1. read_instance / write_instance -- the SAE instance.txt format, ported
   natively from the legacy reader so the package needs nothing outside this
   folder. Same file schema across regions (confirmed): sections
...
3. make_lottery / resample_lottery -- the tie-breaking rules:
       "individual" -> one draw per student, used at every school (MTB as run
                       in the paper's individual tables)
       "family"     -> one draw per family at EVERY school (STB-F), plus the
                       sigma_f offsets
       "mtbf"       -> one draw per (family, school), shared by all family
                       members at that school across levels, plus the sigma_f
                       offsets (paper footnote 4). This is the family-level
                       tie-break of Tables 1-3. The legacy rank-converted
                       codebase path is intentionally NOT reproduced here: it
                       was only order-faithful within a (school, level) and
                       broke Proposition 6 for partial priorities.
"""
...
from v2_model import Instance, School, Level, Student


# ==========================================================================
# instance.txt format (native port of the legacy reader/writer)
# ==========================================================================
...
def read_instance(filename: str):
    ...
    exactly like the legacy reader. Blank lines are skipped; sections end at
...
def write_instance(students, colleges, pref, cap, siblings, levels_map,
    ...
    """Emit the same format the legacy writer produced (round-trippable)."""
