...
The integer programs in v5_exact.py carve out exactly the set these predicates
describe. For partial priorities that claim is certified inside this archive by
v5_audit_partial.py against the independent oracle in v5_certify_partial.py;
for absolute priorities it is certified by exhaustive enumeration in the v1
archive, which contains the original test suite.
...
standalone checker; they are handled in v5_exact.py. A matching returned by the
...
from v5_model import (Instance, Matching, School, Student, is_feasible,
                   occupants_by_slot)
...
def is_contingent_stable(inst: Instance, mu: Matching, priority: str = "absolute",
                         return_blocking: bool = False, ann=None):
    ...
    # callers that already hold the annotation for (inst, mu, priority) may
    # pass it in to avoid recomputing it; the default recomputes as before
    if ann is None:
