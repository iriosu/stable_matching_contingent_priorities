...
if __name__ == "__main__":
    ...
    objective = "SOSM"
    sim = 44
    indir = dropbox_dir + os.sep + "Data" + os.sep + region + os.sep + "2023"
    tb = pickle.load(open(indir + os.sep + "tb_" + tie_breaker + ".pck", "rb"))
    ...
    # Previous tests with Magallanes
    # tie_breaker = "mtbf"
    ...
    out_abs = AbsoluteHard(
        (students, colleges, pref, cap, siblings, levels, students_per_level, Tp, Tn, Sp, Sn, tb),
        "last_pref",
        outdir,
    )

    out_abs_soft = AbsoluteSoft(
        (students, colleges, pref, cap, siblings, levels, students_per_level, Tp, Tn, Sp, Sn, tb),
        "last_pref",
        outdir,
    )
    ...
    out_nosib = NoSiblings(
        (students, colleges, pref, cap, siblings, levels, students_per_level, Tp, Tn, Sp, Sn, tb),
        "last_pref",
        outdir,
    )

    out_DA_desc = alg.Sequential(
        (students, colleges, pref, cap, siblings, levels, students_per_level)
    )

    out_NTB = AbsoluteHardNTB(
        (students, colleges, pref, cap, siblings, levels, students_per_level, Tp, Tn, Sp, Sn, tb),
        "last_pref",
        outdir,
    )
