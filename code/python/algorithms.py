...
def Sequential(inputs, levels_to_process=None):
    def UpdatePriorities(in_match, colleges, pref, siblings, siblings_priority):
        ...
        for c in colleges:
            if c not in out_pref:
                continue
            sorted_values = sorted(out_pref[c].items(), key=lambda item: (-siblings_priority[item[1]][c], item[0] ))
    ...
    for idx in levels_to_process:
        ...
        pref_in_level = {idx: pref_updated[idx] for idx in students_and_schools_in_level if idx in pref_updated}
        match[idx] = DA(students_in_level,pref_in_level,cap_in_level)

        # update priorities of all siblings in coming levels
        pref_updated, siblings_priority = UpdatePriorities(match[idx], colleges, pref, siblings, siblings_priority)

    x_opt = {id_s: {match[idx][id_s]:1} for idx in match for id_s in match[idx] if match[idx][id_s] is not None}
    runtime = time.time()-stime
    outputs = {'status': 'completed','x_opt':x_opt, 'runtime':runtime, 'num_vars':0, 'num_cols':0, 'mipgap':0, 'nodes':0}
    return outputs
...
def SequentialBlock(inputs, levels_to_process=None):
    def UpdatePriorities(in_match, students, colleges, pref, siblings, siblings_priority):
        ...
        for s in students:
            if s not in out_pref:
                continue
            sorted_values = sorted(out_pref[s].items(), key=lambda item: (-siblings_priority[s][item[1]], item[0] ))
            sorted_values_only = [value for key, value in sorted_values]
    ...
    for idx in levels_to_process:
        ...
        # update priorities of all siblings in coming levels
        pref_updated, siblings_priority = UpdatePriorities(match[idx], students, colleges, pref, siblings, siblings_priority)
...
if __name__ == '__main__':
    ...
    indir = dropbox_dir + os.sep + 'Data' + os.sep + 'Magallanes All merged'
    ...
    students, colleges, pref, cap, siblings, levels, students_per_level, S, T, L = genin.read_instance(indir + os.sep + 'instance_'+tie_breaker+'.txt')

    outputs = SequentialBlock((students, colleges, pref, cap, siblings, levels, students_per_level))
