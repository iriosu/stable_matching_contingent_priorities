...
def clean_reserves(students, pref, colleges, cap):
    out_pref = {}
    for c in colleges:
        if c not in pref:
            continue
...
def clean_reserves_and_merge_schools(students, pref, colleges, cap):
...
def write_instance_from_csv(indir, outdir, tie_breaker="mtbf", yr=2023):
    ...
    students, pref, colleges, cap, siblings, levels, students_per_level = (
        clean_reserves_and_merge_schools(students, pref, colleges, cap)
    )
...
if __name__ == "__main__":
    ...
    indir = dropbox_dir + os.sep + "Data" + os.sep + region
    ...
    # old data generation
    # mat = pickle.load(open(indir + os.sep + "mat.pck", "rb"))
    # pref = pickle.load(open(indir + os.sep + "pref.pck", "rb"))
    # cap = pickle.load(open(indir + os.sep + "cap.pck", "rb"))

    # colleges = list(cap.keys())
    # students = list(set(pref.keys()) - set(colleges))
    # siblings = read_siblings(indir + os.sep + "siblings.csv", students)

    # # students, pref, colleges, cap, siblings = clean_reserves(students, pref, colleges, cap)
    # # levels = create_levels(colleges)
    # # students_per_level = create_students_per_level(students, pref)
    ...
    #     levels,
    #     students_per_level,
    #     indir + os.sep + "instance.txt",
    # )

    # tie_breaker = "stbf"
    # pref, tb = modify_school_loterries(pref, students, colleges, siblings, tie_breaker)

    # with open(indir + os.sep + "tb_" + tie_breaker + ".pck", "wb") as f:
    #     pickle.dump(tb, f)
    ...
    #     levels,
    #     students_per_level,
    #     indir + os.sep + "instance_" + tie_breaker + ".txt",
    # )

    # students = ["a1", "a2", "a3", "a4", "a5", "a6", "s", "sp"]
    # colleges = ["c1_g", "c2_g"]
    # siblings = {
    #     "s": ["sp"],
    #     "sp": ["s"],
    ...
    # }
    # levels = {"g": ["c1_g", "c2_g"]}
    # students_per_level = {"g": students}

    # pref = {
    #     "c1_g": {1: "a1", 2: "s", 3: "a2", 4: "sp", 5: "a3"},
    #     "c2_g": {1: "a6", 2: "a3", 3: "a5", 4: "a4", 5: "sp", 6: "s"},
    #     "a1": {1: "c1_g"},
    #     "a2": {1: "c1_g"},
    #     "a3": {1: "c1_g", 2: "c2_g"},
    #     "a4": {1: "c2_g"},
